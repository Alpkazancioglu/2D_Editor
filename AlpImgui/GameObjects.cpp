...
Enum_WarningStatus CreateNewObject(bool& active, std::map<std::string, GameObject>& object)
{
	...
	auto Textures = FilePath::ReadMultipleFilesFromDirectory(FilePath::GetRelativeTexturePath().c_str(),".png");
	...
	if(ImGui::Button("Create"))
	{
		...
		if(object.find(ObjectName) == object.end())
		{
			...
			else
			{
				...
				Iterator++;
				active = false;
				object[ObjectName].Data.TextureName = TextureName;
...
void UpdateTextureSize(std::map<std::string, GameObject>& objects)
{
	for(auto& object : objects)
	{
		object.second.Texture.width = object.second.Data.OriginalTextureSize.x * object.second.Data.TextureScale;
		object.second.Texture.height = object.second.Data.OriginalTextureSize.y * object.second.Data.TextureScale;
	}
...
void MakeVariablesTheSame(std::map<std::string, GameObject>& objects, GameObject*& SelectedObject)
{
	for(auto& object : objects)
	{
		object.second.Global.ShowAllHitboxs = SelectedObject->Global.ShowAllHitboxs;
		object.second.Global.ShouldObjectOrHitboxMove = SelectedObject->Global.ShouldObjectOrHitboxMove;
		object.second.Global.MoveValue = SelectedObject->Global.MoveValue;
	}
